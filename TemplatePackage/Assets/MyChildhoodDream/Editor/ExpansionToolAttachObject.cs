...
//  <created date>11/02/2017</date>
//  <update date>11/02/2017</date>
//  <summary>アタッチしたオブジェクトを起点に処理を行う</summary>
// --------------------------------------------------------------------------------------------------------------------
namespace MyChildhoodDream.Editor
{
    #region
    ...
    public class ExpansionToolAttachObject : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("オブジェクト名リストを作成する")) {
                ...
                List<GameObject> objList = this.attachObject.GetAll();
                foreach (GameObject obj in objList) outputText += obj.name + "\r\n";
            ...
            if (GUILayout.Button("オブジェクト名の変数リストを作成する(全部入り)")) {
                ...
                List<GameObject> objList = this.attachObject.GetAll();
                foreach (GameObject obj in objList) outputText += this.ConvertSerializeFieldText(obj);
            ...
            if (GUILayout.Button("リセット文を作成する(全部入り)")) {
                ...
                List<GameObject> objList = this.attachObject.GetAll();
                foreach (GameObject obj in objList) outputText += this.ConvertResetText(obj);
            ...
            if (GUILayout.Button("オブジェクト名の変数リストを作成する(オブジェクトのみ)")) {
                ...
                List<GameObject> objList = this.attachObject.GetAll();
                foreach (GameObject obj in objList) outputText += this.ObjectOnilyConvertSerializeFieldText(obj);
            ...
            if (GUILayout.Button("リセット文を作成する(オブジェクトのみ)")) {
                ...
                List<GameObject> objList = this.attachObject.GetAll();
                foreach (GameObject obj in objList) outputText += this.ObjectOnilyConvertResetText(obj);
        ...
        /// <param name="currentObject">アタッチしているオブジェクト</param>
        /// <returns>変換後のテキストを挿入済みの画面に表示するテキスト</returns>
        private string ObjectOnilyConvertResetText(GameObject currentObject)
        {
            string outputText = "    this." + this.ConvertFirstTextLowerCase(currentObject.name) + "Object = GameObject.Find(\"" + currentObject.name + "\").gameObject;\r\n\r\n";
            return outputText;
        }
        ...
        /// <param name="currentObject">アタッチしているオブジェクト</param>
        /// <returns>変換後のテキストを挿入済みの画面に表示するテキスト</returns>
        private string ObjectOnilyConvertSerializeFieldText(GameObject currentObject)
        {
            string outputText = "[SerializeField] private GameObject " + this.ConvertFirstTextLowerCase(currentObject.name) + "Object;\r\n\r\n";
            return outputText;
        }
